...
export default {
  ...
  set_app: (app_id, data) =>
    api.post(app_id ? '/apps/' + app_id : '/apps', data),
  app_new_secret: (app_id) => api.post('/apps/' + app_id + '/secret'),
  app_new_session: (app_id, pin, session_secret) =>
    api.post(`/apps/${app_id}/session`, { pin, session_secret }),
  ...
  transactions: (data, token) =>
    api.post('/transactions', data, getHeader(token)),
  search: (identity_number, token) =>
    api.get('/search/' + identity_number, getHeader(token)),
  check_user: (user_id, token) =>
    api.get('/users/' + user_id, getHeader(token)),
}
