...
export default {
  ...
  mounted() {
    !(this.active_app.app_id || this.$route.params.app_number) &&
...
async function _request_new_secret() {
  if (once_submit)
    return this.$message.error({
      message: this.$t('message.errors.reset'),
      showClose: true,
    })
  this.loading = true
  once_submit = true
  try {
    ...
    this.$message.success({
      message: this.$t('message.success.reset'),
      showClose: true,
    })
...
async function _request_new_session(algo = 'rsa') {
  ...
  this.loading = true
  try {
    const res = await this.apis.app_new_session(
  ...
  function _get_ed25519_private_key() {
    ...
    let session_secret = keypair.publicKey
      .toString('base64')
      .replaceAll('+', '-')
      .replaceAll('/', '_')
      .replace(/=+$/, '')
...
function _download_app_json() {
  ...
  const dataStr =
    'data:text/json;charset=utf-8,' + encodeURIComponent(this.modal_content)
