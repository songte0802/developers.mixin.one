  ...
  <div class="container">
    <div v-loading="loading" class="secret-list f">
      <div class="item">
        <div class="secret-item">
          ...
          <button @click="request_new_secret" class="primary">
            {{ $t('secret.secret_btn') }}
          </button>
        </div>
      </div>
      <div class="item">
        <div class="secret-item">
          ...
          <div class="secrets">
            <button @click="request_new_session" class="primary">
              {{ $t('secret.session_btn') }}
            </button>
            <button @click="request_ed25519_session" class="primary">
              {{ $t('secret.session_ed25519_btn') }}
            </button>
          </div>
        </div>
      </div>
      <div class="item">
        <div class="secret-item qrcode">
          ...
          <div class="qrcode-btns">
            <button @click="request_show_qrcode" class="primary">
              {{ $t('secret.qrcode_btn1') }}
            </button>
            <button @click="request_rotate_qrcode" class="primary">
              {{ $t('secret.qrcode_btn2') }}
            </button>
          </div>
        </div>
      </div>
    ...
    <d-modal class="secret-modal" :show="modal_content ? true : false">
      <div class="new-secret-modal">
        <h3>{{ modal_title }}</h3>
        <span :class="tmp_action === 'session' && 'session'">{{
          modal_content
        }}</span>
        <div class="btns">
          <button
            v-if="tmp_action === 'secret'"
          ...
          <button
            v-if="tmp_action === 'session'"
            @click="click_download_session"
            class="btn-close primary"
          >
            {{ $t('button.download') }}
          </button>
