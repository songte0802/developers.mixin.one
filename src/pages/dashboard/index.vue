<template>
  <div
    ...
    :class="['development-dashboard', $t('language')]"
  >
    <template v-if="init_status">
      <d-header v-if="$route.name !== 'dashboard'" class="app-header">
        ...
        <div slot="center">
          {{ active_app.name || $t('dashboard.new_app') }}
        </div>
      </d-header>
      <d-header v-else-if="is_mobile">
        <div :slot="!is_immersive ? 'left' : 'center'">
          {{ $t('dashboard.title') }}
        </div>
        <div :slot="!is_immersive ? 'right' : 'left'">
          ...
          <div
            :class="[
              'right-more',
              show_click_user ? 'right-more-active' : '',
              !is_immersive ? 'header-slot-right' : 'header-slot-left',
            ]"
          >
            <ul class="right-button-list">
              <li @click.stop="click_sign_out" class="right-button-item">
                <img
                  class="icondengchu"
                  src="@/assets/img/app-svg/logout.svg"
                />
      ...
      <div class="dashboard-container">
        <nav>
            ...
            <div class="app-list-container" v-if="app_list.length">
              ...
              <ul class="app-list">
                <li
                  ...
                  :class="[
                    'app-item',
                    active_app.app_id === item.app_id ? 'app-item-active' : '',
                  ]"
                >
          ...
          <div @click.stop="click_user" class="bottom-info middle">
            ...
            <div
              :class="[
                'bottom-more',
                show_click_user ? 'bottom-more-active' : '',
              ]"
            >
        ...
        <div
          v-if="$route.name !== 'dashboard' || !is_mobile"
          class="dashboard-center-and-nav"
        >
          <div v-if="is_welcome" class="welcome">
            <img src="@/assets/img/svg/robot.svg" />
            <h1>{{ $t('dashboard.welcome') }}</h1>
            <p>{{ $t('dashboard.welcome_d') }}</p>
            <button @click="click_new_app" class="primary">
            ...
            <header>
              <div :class="['header-list', 'header-index-' + nav_header_index]">
                <template v-if="is_new_app">
                  <span class="header-item new-item">{{
                    $t('dashboard.new_app')
                  }}</span>
                </template>
                <template v-else>
                  <span
                    ...
                    :class="[
                      'header-item',
                      nav_header_index === index ? 'header-item-active' : '',
                    ]"
    ...
    <d-modal :show="balance_modal">
      <div class="edit-modal">
        ...
        <button @click="click_buy_item(1)" class="btns-save primary">
          {{ $t('dashboard.buy.btn', { count: 1 }) }}
        </button>
        <button @click="click_buy_item(2)" class="btns-save primary">
          {{ $t('dashboard.buy.btns', { count: 2 }) }}
        </button>
        <button @click="click_buy_item(5)" class="btns-save primary">
