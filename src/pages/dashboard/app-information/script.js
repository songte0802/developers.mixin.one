  ...
  methods: {
    ...
    init_app(app) {
      ...
      if (resource_patterns)
        this.resource_patterns =
          resource_patterns && resource_patterns.join('\n')
      if (capabilities)
        this.immersive_status =
          capabilities && capabilities.includes('IMMERSIVE')
      _check_is_finished.call(this)
    },
  },
  mounted() {
...
async function _submit_to_database() {
  ...
  let capabilities = this.immersive_status
    ? ['CONTACT', 'GROUP', 'IMMERSIVE']
    : ['CONTACT', 'GROUP']
  let parmas = {
    capabilities,
    description,
  ...
  try {
    ...
    if (res && res.type === 'app') {
      this.$message.success({
        message: this.$t('message.success.save'),
        showClose: true,
      })
...
function notice() {
  ...
  if (app_name.length < 2 || app_name.length > 64)
    return notice_message.call(this, 'app_name_length')
  if (description.length < 16 || description.length > 128)
    return notice_message.call(this, 'description_length')
}

function notice_message(message) {
  return this.$message.error({
    message: this.$t('information.errors.' + message),
    showClose: true,
  })
}
