      ...
      <transition name="fade-up">
        <div v-if="show && !snap_status" v-loading="loading" class="main">
          ...
          <div class="content">
            ...
            <footer>
              <div class="btns">
                <button @click="click_submit" class="btns-copy primary">
                  {{ $t('button.withdrawal') }}
                </button>
...
async function submit_withdrawal() {
  ...
  let params = await _build_transfers_params.call(
    this,
    client_info,
    is_transfers
  )
...
async function _build_transfers_params(client_info, is_transfers) {
  ...
  opponent_id = is_transfers
    ? await _get_opponent_id.call(this, client_info)
    : opponent_id
  if (!opponent_id)
    return this.$message.error({
      message: this.$t('message.errors.mixin_id'),
      showClose: true,
    })
  const { pinToken, privateKey, sid } = client_info
  const pin = this.tmp_pin
  const opponent = opponent_id.startsWith('XIN')
